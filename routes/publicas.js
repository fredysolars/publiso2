...
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user:'fredysolars@gmail.com',
    pass:'Mundial_25252010'
  }
})
      ...
      if (filas.length > 0) {
        ...
        respuesta.redirect('/registro')
            ...
            const consulta = `
                                INSERT INTO
                                propietarios
                                (email, contrasena, nombres, apellidos, celular)
                                VALUES (
                                  ${connection.escape(email)},
                                  ${connection.escape(contrasena)},
                                  ${connection.escape(nombres)},
                                  ${connection.escape(apellidos)},
                                  ${connection.escape(celular)}
                                )
                              `
            connection.query(consulta, (error, filas, campos) => {
...
router.get('/qs', (peticion, respuesta) => {
  respuesta.render('qs', { mensaje: peticion.flash('mensaje') })
})
...
router.get('/negocios/:id', (peticion, respuesta) => {
  pool.getConnection((err, connection) => {
    ...
    connection.query(consulta, (error, filas, campos) => {
      if (filas.length > 0) {
        respuesta.render('publicacion', { negocios: filas[0] })
      }
      else {
        respuesta.redirect('/')
      }
